...
// Простая транслитерация кириллицы для имён файлов
...
// Нормализация имени файла для отчёта
function normalizeReportName(title: string, date?: Date): string {
  const dateStr = date
  ...
  const normalized = transliterate(title)
    ...
    .replace(/_+/g, '_')
    .replace(/^_|_$/g, '')
    .slice(0, 80); // Ограничиваем длину
  return `${normalized}_${dateStr}`;
}
...
export type AnalyzeTrackOptions =
  | { analyzer?: AnalyzerType; drumsPath?: string; reportName?: string; reportDate?: Date }
  | boolean;
...
export async function analyzeTrack(
  ...
  const reportName =
    typeof options === "object" && options !== null && "reportName" in options
      ? (options as { reportName?: string }).reportName
      : undefined;
  const reportDate =
    typeof options === "object" && options !== null && "reportDate" in options
      ? (options as { reportDate?: Date }).reportDate
      : undefined;
  ...
  try {
    ...
    if (useCorrelation && result.row_analysis) {
      try {
        ...
        // Используем переданное имя трека или fallback на имя файла
