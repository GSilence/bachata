/**
 * GET  /api/playlists/favorites/tracks — список ID треков в Избранном
 * POST /api/playlists/favorites/tracks — добавить трек { trackId }
 * DELETE /api/playlists/favorites/tracks?trackId=X — удалить трек
 */
...
async function getFavoritesPlaylist(prisma: any, userId: number) {
  let playlist = await prisma.playlist.findFirst({
    where: { userId, type: "favorites" },
  });
  if (!playlist) {
    // Создаём на лету, если по какой-то причине не было создано при регистрации
    playlist = await prisma.playlist.create({
      data: { userId, name: "Избранное", type: "favorites", isSystem: true },
    });
  }
  return playlist;
}
...
export async function GET(request: NextRequest) {
  ...
  const playlist = await getFavoritesPlaylist(prisma, auth.userId);
