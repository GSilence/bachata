...
export default function TrackInfo({}: TrackInfoProps) {
  ...
  return (
    <div
      className="bg-gray-800 p-4 sm:p-6 border border-gray-700"
      data-component="track-info"
    >
      {/* Название, исполнитель, год, альбом — для всех (только чтение) */}
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          {/* Title + star + status in one row */}
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <h2 className="text-xl sm:text-2xl font-bold text-white">
              {currentTrack.metaTitle || currentTrack.title}
            </h2>
            {isAdminUser && (
              <>
                <button
        ...
        {/* Right: "не попса" + edit + [24px gap] + delete */}
        {isAdminUser && (
          <div className="flex items-center shrink-0">
            {currentTrack?.trackStatus === "popsa" && (
              <button
                onClick={handleMarkNotPopsa}
                disabled={isMarkingNotPopsa}
                className="px-2 py-1 text-xs text-orange-400 hover:text-orange-300 hover:bg-orange-900/20 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed border border-orange-800/40 mr-1"
            ...
            <button
              ...
              className="p-1.5 rounded text-amber-400 hover:text-amber-300 hover:bg-amber-900/20 transition-colors"
            ...
            <button
              ...
              disabled={isDeleting}
              className="p-2 ml-4 text-red-400 hover:text-red-300 hover:bg-red-900/20 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              title="Удалить трек"
