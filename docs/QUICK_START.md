# Быстрый старт - установка через консоль

Самая быстрая установка Bachata Beat Counter на VDS Timeweb через веб-консоль.

## 🚀 Один скрипт - всё автоматически

Скопируйте и выполните эти команды в веб-консоли Таймвеба:

```bash
# Скачиваем скрипт
wget https://raw.githubusercontent.com/GSilence/bachata/main/install-bachata.sh -O install-bachata.sh

# Или через curl (если wget не работает)
curl -o install-bachata.sh https://raw.githubusercontent.com/GSilence/bachata/main/install-bachata.sh

# Делаем исполняемым
chmod +x install-bachata.sh

# Запускаем установку
./install-bachata.sh
```

**Всё!** Скрипт автоматически:

- ✅ Проверит, что уже установлено
- ✅ Установит все зависимости
- ✅ Настроит базу данных
- ✅ Создаст все необходимые директории
- ✅ Настроит Nginx и systemd сервис

## 📝 Что делает скрипт

1. Обновляет систему
2. Устанавливает Node.js 20, Python 3.10, MySQL, Nginx
3. Создает базу данных и пользователя
4. Настраивает Python виртуальное окружение
5. Устанавливает Python зависимости (madmom, demucs) - **10-20 минут**
6. Создает пользователя приложения и директории
7. Настраивает Nginx как reverse proxy
8. Создает systemd сервис
9. Настраивает firewall

## 📦 После установки

### 1. Загрузите проект на сервер

```bash
# Через Git
cd /opt/bachata
sudo -u bachata git clone https://github.com/GSilence/bachata.git .

# Или через SCP с вашего компьютера
scp -r ./* root@your-server-ip:/opt/bachata/
chown -R bachata:bachata /opt/bachata
```

### 2. Запустите финальную настройку

Если проект уже загружен, скрипт автоматически:

- Установит Node.js зависимости
- Настроит Prisma
- Соберет Next.js приложение
- Запустит сервис

Если проект еще не загружен, после загрузки выполните:

```bash
cd /opt/bachata
sudo -u bachata npm install
sudo -u bachata npm run db:generate
sudo -u bachata npm run db:push
sudo -u bachata npm run build
systemctl start bachata-beat-counter
```

### 3. Загрузите голосовые файлы

```bash
# С вашего компьютера
scp public/audio/voice/1.mp3 public/audio/voice/2.mp3 ... root@your-server-ip:/opt/bachata/public/audio/voice/

# На сервере установите права
chown -R bachata:bachata /opt/bachata/public/audio/voice
```

**Важно:** Нужны только файлы `1.mp3` - `8.mp3` (не 9-10).

## ✅ Проверка установки

```bash
# Статус сервиса
bachata-manage status

# Логи
bachata-manage logs

# Доступ к приложению
# Откройте в браузере: http://your-server-ip
```

## 🔄 Повторный запуск

Скрипт можно запускать многократно! Он автоматически:

- Пропустит уже установленные компоненты
- Проверит существующие настройки
- Выполнит только необходимые шаги

## 📋 Управление приложением

```bash
bachata-manage start    # Запустить
bachata-manage stop     # Остановить
bachata-manage restart  # Перезапустить
bachata-manage status   # Статус
bachata-manage logs     # Логи
bachata-manage rebuild  # Пересобрать
```

## 🔍 Логи установки

Все логи сохраняются в: `/var/log/bachata-install.log`

```bash
# Просмотр логов
tail -f /var/log/bachata-install.log
```

## 🔑 Пароль базы данных

Пароль БД сохраняется в: `/root/db_credentials.txt`

```bash
cat /root/db_credentials.txt
```

## ❓ Проблемы?

См. подробные инструкции:

- [`docs/MANUAL_INSTALL_TIMEWEB.md`](./MANUAL_INSTALL_TIMEWEB.md) - пошаговая ручная установка
- [`docs/INSTALL_ON_EXISTING_VDS.md`](./INSTALL_ON_EXISTING_VDS.md) - установка через скрипт
- [`docs/DEPLOYMENT_VDS.md`](./DEPLOYMENT_VDS.md) - полная документация

## 📝 Примечания

- Установка Python зависимостей (madmom) может занять **10-20 минут** - это нормально
- Скрипт работает даже если проект еще не загружен
- Можно запускать многократно - скрипт идемпотентный
- Все проверки выполняются автоматически
